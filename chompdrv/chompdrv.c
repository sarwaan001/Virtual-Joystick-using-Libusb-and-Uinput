...
int setup_uinput_device() {
    ...
    if(err < 0) {
        printf("IOCTL EV_KEY ERROR");
    }
    err = ioctl(uinp_fd, UI_SET_EVBIT, EV_SYN);
    if(err < 0) {
        printf("IOCTL EVBIT ERROR\n");
    }
...
int main() {
    ...
    if (dev == NULL) {
        printf("DEVICE NOT FOUND. PLEASE USE USBIP AND TRY AGAIN.");
        return -1;
    }
	...
	//converting to bits - found on stack overflow
	...
	// Used for Debugging
	/*
	printf("XSTATE: %d", XState);
	printf(" YSTATE: %d", YState);
